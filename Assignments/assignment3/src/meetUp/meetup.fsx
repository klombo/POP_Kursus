...
module MeetUp =
    ...
    type Skeptic(name: string, politicalview: float, influenciable: float, rizz: float) =
        ...
        override this.OnInteraction other isAMatch =
            let difference = other.politicalView - this.politicalView
            let influenceStrength  = this.influenciable * other.rizz
            difference * influenceStrength * -1.0
    
    let participantList: Participant list = [
        CopyCat("David Attenborough", 0.01, 0.01, 0.7);
        CopyCat("Linda Hamilton",0.36, 0.04, 0.2);
        Skeptic("Jesus Christus", 0.2, 0.07, 1.0);
        Normal("Thomas Tog", 0.09, 0.06, 0.0);
        CopyCat("Jessica Rabbit", 0.63, 0.01, 0.0);
        Normal("Christopher Columbus", 0.98, 0.05, 0.1);
        Skeptic("Elizabeth Turner", 0.89, 0.04, 0.02);
        Normal("Dan Turrell", 0.78, 0.08, 0.01);
        Skeptic("Charlie Kirk", 1.0, 0.0, 1.0);
        Normal("Queen Elizabeth", 0.90, 0.04, 0.50)
    ]
    ...
    let mutable edgeMatrix = createEdgeMatrix participantList
    let g0 : Graph<Participant> = empty

    let g1 =
        participantList
        |> List.fold (fun g v -> addVertex v g) g0
    ...
    let simulateMeeting(participantList : Participant List) =
        let [p1;p2] = participantList |> List.randomSample 2
        let isAMatch = p1.MatchScore p2 > 0.5
        printfn "navn: %s, policticalview: %A" p1.name p1.politicalView
        printfn "navn: %s, policticalview: %A" p2.name p2.politicalView
        p1.politicalView <- p1.politicalView - p1.OnInteraction p2 isAMatch
        p2.politicalView <- p2.politicalView - p2.OnInteraction p1 isAMatch
        printfn "navn: %s, policticalview: %A" p1.name p1.politicalView
        printfn "navn: %s, policticalview: %A" p2.name p2.politicalView

    
    simulateMeeting(participantList)
    simulateMeeting(participantList)
    simulateMeeting(participantList)
    simulateMeeting(participantList)
    simulateMeeting(participantList)
